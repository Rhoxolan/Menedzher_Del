...
void getin(deed* deal, short number, wstring* myfile_title, wstring* myfile_writeup, wstring* myfile_priority, wstring** myfile_date, wstring** myfile_time, wstring* myfile_dealnumber)
{
	//Ввод данных в "дело"
	ofstream deedout;
	cout << "Пожалуйста, введите название дела (0 - выход в главное меню): ";
	cin.ignore();
	...
	if (deal[number].title[0] != '0')
	{
		...
		if (!deedout.is_open())
		{
			cout << "\n\aОшибка открытия программного файла! Данные могут не сохраниться.\n\n";
		}
		else
		{
		...
		if (!deedout.is_open())
		{
			cout << "\nОшибка открытия программного файла! Данные могут не сохраниться.\n\n";
		}
		else
		{
...
void AddNewDeed(bool* numid, short number, deed* deal, wstring* myfile_title, wstring* myfile_writeup, wstring* myfile_priority, wstring** myfile_date, wstring** myfile_time, wstring* myfile_dealnumber)
{
		...
		do
		{
			if (numid[x] == false)
			{
				...
				if (deal[x].title[0] != '0')
				{
					...
					if (!deedout.is_open())
					{
						cout << "Ошибка открытия программного файла! Данные могут не сохраниться.";
					}
					else
					{
...
void DeedDelete(bool* numid, wstring* myfile_title, wstring* myfile_writeup, wstring* myfile_priority, wstring** myfile_date, wstring** myfile_time, wstring* myfile_dealnumber)
{
	//Удаление дел
	short remdeal;
	do
	{
		...
		{
			short* hidefewdeeds = new short[100 + 1]; //Добавляем 1 элемент для hidefewdeeds[101] в цикле do while ниже
			short hideid = -1;
			bool* anynumid = new bool[100 + 1]; //Добавляем 1 элемент для anynumid[hidefewdeeds[101]] в цикле do while ниже
			for (short l = 0; l <= 99 + 1; l++)
			{
				anynumid[l] = true;
			}
			...
			{
				hideid++;
				while (!(cin >> hidefewdeeds[hideid]) || hidefewdeeds[hideid] < 0 || hidefewdeeds[hideid] > 100 || anynumid[hidefewdeeds[hideid]] == false)
				{
					if (!cin)
					{
						cin.clear();
					...
					cout << "Вы ввели недопустимое значение или ввели значение повторно. Пожалуйста, повторите ввод: ";
				}
				anynumid[hidefewdeeds[hideid]] = false;
			...
			if (hidefewdeeds[0] != 0)
			{
				...
				if (showhide == true)
				{
					cout << "Успешно удалены следующие дела: ";
					for (short y = 0; y <= hideid - 1; y++)
					{
						if (numid[hidefewdeeds[y] - 1] == true && hidefewdeeds[y] != 0)
						{
							cout << hidefewdeeds[y] << " ";
						}
					}
					for (short e = 0; e <= hideid - 1; e++)
					{
						...
						_wremove(myfile_title[hidefewdeeds[e] - 1].c_str());
						_wremove(myfile_writeup[hidefewdeeds[e] - 1].c_str());
						_wremove(myfile_priority[hidefewdeeds[e] - 1].c_str());
						_wremove(myfile_date[hidefewdeeds[e] - 1][2].c_str());
						_wremove(myfile_date[hidefewdeeds[e] - 1][1].c_str());
						_wremove(myfile_date[hidefewdeeds[e] - 1][0].c_str());
						_wremove(myfile_time[hidefewdeeds[e] - 1][1].c_str());
						_wremove(myfile_time[hidefewdeeds[e] - 1][0].c_str());
						_wremove(myfile_dealnumber[hidefewdeeds[e] - 1].c_str());
					}
				}
				else
				{
			...
			while (hidedeal != 0)
			{
				...
				else
				{
					if (numid[hidedeal - 1] == true)
					{
						...
						_wremove(myfile_title[hidedeal - 1].c_str());
						_wremove(myfile_writeup[hidedeal - 1].c_str());
						_wremove(myfile_priority[hidedeal - 1].c_str());
						_wremove(myfile_date[hidedeal - 1][2].c_str());
						_wremove(myfile_date[hidedeal - 1][1].c_str());
						_wremove(myfile_date[hidedeal - 1][0].c_str());
						_wremove(myfile_time[hidedeal - 1][1].c_str());
						_wremove(myfile_time[hidedeal - 1][0].c_str());
						_wremove(myfile_dealnumber[hidedeal - 1].c_str());
		...
		/*удаление всех дел*/else if (remdeal > 0)
		{
			...
			else
			{
				...
				if (choice == 1)
				{
					...
					{
						numid[y] = false;
						_wremove(myfile_title[y].c_str());
						_wremove(myfile_writeup[y].c_str());
						_wremove(myfile_priority[y].c_str());
						_wremove(myfile_date[y][2].c_str());
						_wremove(myfile_date[y][1].c_str());
						_wremove(myfile_date[y][0].c_str());
						_wremove(myfile_time[y][1].c_str());
						_wremove(myfile_time[y][0].c_str());
						_wremove(myfile_dealnumber[y].c_str());
					}
...
void RedactingTheDeed(bool* numid, short number, deed* deal, wstring* myfile_title, wstring* myfile_writeup, wstring* myfile_priority, wstring** myfile_date, wstring** myfile_time, wstring* myfile_dealnumber)
{
	//Редактирование дел
	ofstream deedout;
		...
		else
		{
			if (numid[editthedeal - 1] == true)
			{
				...
				do
				{
					...
					{
						deed* temporarytitle = new deed[1]; //Создаём временный массив для сохранения заголовка "дела" в случае отказа от редактирования
						temporarytitle[0] = deal[number];
						cout << "Дело № " << editthedeal << ":\n";
						show(deal, number);
						cout << endl;
					...
					/*редактирование даты и времени*/else if (redact > 3)
					{
						...
						if (!deedout.is_open())
						{
							cout << "\n\aОшибка открытия программного файла! Данные могут не сохранится.\n\n";
						...
						if (!deedout.is_open())
						{
							cout << "\nОшибка открытия программного файла! Данные могут не сохранится.\n\n";
						}
						else
						{
							deedout << deal[number].date[1];
						...
						if (!deedout.is_open())
						{
							cout << "\nОшибка открытия программного файла! Данные могут не сохранится. Нажмите любую клавишу для продолжения\n\n";
							system("pause");
						}
						else
						{
					...
					/*редактирование приоритета*/else if (redact > 2)
					{
						...
						if (!deedout.is_open())
						{
							cout << "\n\aОшибка открытия программного файла! Данные могут не сохранится. Нажмите любую клавишу для продолжения\n\n";
							system("pause");
						}
						else
						{
...
int main()
{
	...
	wstring* myfile_title = new wstring[100]; //string* myfile_.. - указание пути к файлу, в котором хранится "дело"
	wstring* myfile_priority = new wstring[100];
	wstring* myfile_writeup = new wstring[100];
	wstring** myfile_date = new wstring * [100];
	for (short initializer = 0; initializer <= 99; initializer++)
	{
		myfile_date[initializer] = new wstring[3];
	}
	wstring** myfile_time = new wstring * [100];
	for (short initializer = 0; initializer <= 99; initializer++)
	{
		myfile_time[initializer] = new wstring[2];
	}
	wstring* myfile_dealnumber = new wstring[100];
	for (short initializer = 0; initializer <= 99; initializer++)
	{
		myfile_title[initializer] = pathtothefolder + L"/title" + to_wstring(initializer + 1);
		myfile_priority[initializer] = pathtothefolder + L"/priority" + to_wstring(initializer + 1);
		myfile_writeup[initializer] = pathtothefolder + L"/writeup" + to_wstring(initializer + 1);
		myfile_date[initializer][0] = pathtothefolder + L"/date[0]" + to_wstring(initializer + 1);
		myfile_date[initializer][1] = pathtothefolder + L"/date[1]" + to_wstring(initializer + 1);
		myfile_date[initializer][2] = pathtothefolder + L"/date[2]" + to_wstring(initializer + 1);
		myfile_time[initializer][0] = pathtothefolder + L"/time[0]" + to_wstring(initializer + 1);
		myfile_time[initializer][1] = pathtothefolder + L"/time[1]" + to_wstring(initializer + 1);
		myfile_dealnumber[initializer] = pathtothefolder + L"/dealnumber" + to_wstring(initializer + 1);
	}
	ifstream deedin;
	short number = 0;
